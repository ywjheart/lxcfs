...
enum {
	...
	LXC_TYPE_PROC_SWAPS,
	LXC_TYPE_SYS_ONLINE, // /sys/devices/system/cpu/online -> /proc/cpu_online, so that we will not break ABI of liblxc.so
...
static char *find_mounted_controller(const char *controller, int *cfd)
{
	...
	// added by yew: fallback to unified if it is blkio/memory controller
	if (strcmp(controller,"blkio") == 0 ||
...
static char *get_pid_cgroup(pid_t pid, const char *contrl)
{
	...
	while (getline(&line, &len, f) != -1) {
		...
		if (!c2)
			goto out;
		*c2 = '\0';
		// added by yew: it is "unified" in hierarchies
		if (!strcmp(c1,"") && !strcmp(h,"unified")){
			// this is unified(cgroupv2)
	...
	while (*memstat) {
		if (startswith(memstat, "total_cache")) {
			...
			*shmem /= 1024;
			...
			// added by yew: cgroupv2 starts here
			...
			*unevictable /= 1024;
		} else if (startswith(memstat, "shmem")) {
			sscanf(memstat + 5, "%lu", shmem);
			*shmem /= 1024;
		}
		eol = strchr(memstat, '\n');
		if (!eol)
...
static int sys_online_read(char *buf, size_t size, off_t offset,
							 struct fuse_file_info *fi)
{
	struct fuse_context *fc = fuse_get_context();
	struct file_info *d = (struct file_info *)fi->fh;
	...
	total_len = snprintf(d->buf, d->size, "%s\n", cpuset);
		...
		struct fuse_file_info *fi)
{
	if (filler(buf, ".", NULL, 0) != 0 ||
		...
		filler(buf, "cpu_online", NULL, 0) != 0 ||
...
int proc_open(const char *path, struct fuse_file_info *fi)
{
	...
	const char* path2 = path; //noted by yew: the dummy path used for size detection
	...
	else if (strcmp(path, "/proc/cpu_online") == 0)
	{
		type = LXC_TYPE_SYS_ONLINE;
		path2 = "/sys/devices/system/cpu/online";
	}
	...
	//noted by yew: path2 is used for dummy file size
	info->buflen = get_procfile_size(path2) + BUF_RESERVE_SIZE;
	do {
		...
		struct fuse_file_info *fi)
{
	...
	case LXC_TYPE_SYS_ONLINE:
		return sys_online_read(buf, size, offset, fi);
